    ...
    private fun readItemFromDatabase()
    {
        ...
        myRef.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                ...
                findViewById<TextView>(R.id.textViewProductName).text = dataSnapshot.child(
                    getString(
                        R.string.path_name
                    )
                ).value.toString()
                findViewById<TextView>(R.id.textView1ProductExpireEdit).text = dataSnapshot.child(
                    getString(
                        R.string.path_exp_date
                    )
                ).value.toString()
                findViewById<TextView>(R.id.textViewProductSizeEdit).text = dataSnapshot.child(
                    getString(
                        R.string.path_size
                    )
                ).value.toString()
                findViewById<TextView>(R.id.textViewProductPriceEdit).text = dataSnapshot.child(
                    getString(
                        R.string.path_price
                    )
                ).value.toString()
                findViewById<TextView>(R.id.textViewProductQtyEdit).text = dataSnapshot.child(
                    getString(
                        R.string.path_qty
                    )
                ).value.toString()
